...
public class FileExceptionManagerTest {
    ...
    public void writeExceptionTest() {
        ...
        ExceptionModel exceptionModel = ExceptionModel.toModel(result);
        Assert.assertEquals(Objects.requireNonNull(exceptionModel).getMessage(), model.getMessage());
    ...
    public void getExceptionTest() {
        ...
        for(int i = 0; i < 2; i++){
            ...
            ExceptionModel model = ExceptionModel.getInstance(new Date(time + (1000 * 60 * (i + 1))), "Ошибка #" + i, "NONE", 0);
            String str = model.toString();
            String fileName = model.getFileName();
            fileExceptionManager.writeBytes(fileName, str.getBytes());
            if(i == 1) {
